...
| `policy_generator.py` | Policy/dashboard/manifest/API generation, Release Health/Atom parsing, safe Atom Support link selection/canonicalization, bounded Support `Applies to` extraction, validated Support article enrichment, exact-token MSRC CVRF enrichment with capped context lists, unique Source Diagnostic IDs, dashboard-only baseline-update notice rendering, and first-party static Pages Wiki/changelog rendering. |
