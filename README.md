...
- Atom is discovery for Support article hrefs; when an Atom KB row lacks a usable `support.microsoft.com` article URL, the generator records Source Diagnostic evidence instead of resolving through `/help/<KB>`. Support article facts are validated against the Atom KB, build, release, and selected URL before they can enrich summaries or provide Support-derived security labels. Security-patch classification comes from exact MSRC CVRF KB evidence or validated explicit Support article wording, not the Atom title alone.
