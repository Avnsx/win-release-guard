...
def _minimal_repo(tmp_path: Path, identity_version: str = "win11_release_guard/0.3.3") -> Path:
...
def test_version_consistency_check_fails_runtime_marker_mismatch(tmp_path: Path) -> None:
    ...
    assert all("expected 'win11_release_guard/0.3.3'" in finding.message for finding in findings)
