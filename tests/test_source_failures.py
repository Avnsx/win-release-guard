...
def _write_signed_policy(path, *, generated_at_utc: str = "2026-05-28T00:00:00+00:00") -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    policy = replace(_generated_policy(), generated_at_utc=generated_at_utc)
...
def test_http_403_404_500_are_structured_and_fall_back(monkeypatch, tmp_path):
    for code, kind in ((403, "http_403"), (404, "http_404"), (500, "http_500")):
...
def test_malformed_json_and_malformed_html_fall_back_to_cache(monkeypatch, tmp_path):
    ...
    assert any(problem.kind == SourceStatus.REMOTE_POLICY_PARSE_FAILED.value.lower() for problem in malformed_json.source_problems)
    assert malformed_html.source_status is SourceStatus.USING_FRESH_CACHE
    assert any(problem.kind == SourceStatus.REMOTE_POLICY_PARSE_FAILED.value.lower() for problem in malformed_html.source_problems)
...
def test_invalid_signature_and_corrupt_cache_are_visible(monkeypatch, tmp_path):
    ...
    assert result.status is EvaluationStatus.CHECK_INCOMPLETE
    assert any(problem.kind == SourceStatus.REMOTE_POLICY_SIGNATURE_FAILED.value.lower() for problem in result.source_problems)
